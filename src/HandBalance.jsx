...
// Hand Balance Visualization - Shows left vs right hand comparison
const HandBalance = ({ fingerStats }) => {
  const [mode, setMode] = useState('speed') // 'speed' or 'accuracy'
  
  // Format accuracy to always show one decimal place
  ...
  const handData = useMemo(() => {
    ...
    const getFingerData = (fingers) => {
      return fingers.map(finger => {
        ...
        const accuracy = data.accuracy !== undefined 
          ? data.accuracy 
          : (data.correct !== undefined && data.total > 0)
            ? Math.round((data.correct / data.total) * 1000) / 10 // One decimal place
            : 0
    ...
    const leftFingerData = getFingerData(leftFingers)
    const rightFingerData = getFingerData(rightFingers)
    
    if (leftFingerData.length === 0 && rightFingerData.length === 0) return null
    
    // Aggregate stats for each hand (weighted by sample count)
    const aggregate = (fingerData) => {
      let totalSamples = 0
      ...
      fingerData.forEach(f => {
        if (f.total > 0) {
          totalSamples += f.total
          weightedIntervalSum += f.avgInterval * f.total
          weightedAccuracySum += f.accuracy * f.total
        }
      })
      
      return {
        avgInterval: totalSamples > 0 ? Math.round(weightedIntervalSum / totalSamples) : 0,
        accuracy: totalSamples > 0 ? Math.round((weightedAccuracySum / totalSamples) * 10) / 10 : 0, // One decimal
        samples: totalSamples,
      }
    }
    
    const left = aggregate(leftFingerData)
    const right = aggregate(rightFingerData)
    
    // Find fastest and slowest fingers per hand
    const findBestWorst = (fingerData, metric, lowerIsBetter) => {
      if (fingerData.length === 0) return { best: null, worst: null }
      const sorted = [...fingerData].sort((a, b) => 
        lowerIsBetter ? a[metric] - b[metric] : b[metric] - a[metric]
      )
      return { best: sorted[0], worst: sorted[sorted.length - 1] }
    }
    
    const leftSpeed = findBestWorst(leftFingerData, 'avgInterval', true)
    const rightSpeed = findBestWorst(rightFingerData, 'avgInterval', true)
    const leftAcc = findBestWorst(leftFingerData, 'accuracy', false)
    const rightAcc = findBestWorst(rightFingerData, 'accuracy', false)
    
    return {
      left,
      right,
      leftFingers: leftFingerData,
      rightFingers: rightFingerData,
      bestSpeed: {
        left: leftSpeed.best,
        right: rightSpeed.best,
      },
      worstSpeed: {
        left: leftSpeed.worst,
        right: rightSpeed.worst,
      },
      bestAccuracy: {
        left: leftAcc.best,
        right: rightAcc.best,
  ...
  const { left, right, bestSpeed, worstSpeed, bestAccuracy, worstAccuracy } = handData
  
  // Determine which hand is better based on mode
  const leftValue = mode === 'speed' ? left.avgInterval : left.accuracy
  const rightValue = mode === 'speed' ? right.avgInterval : right.accuracy
  
  // For speed, lower is better. For accuracy, higher is better.
  const leftIsBetter = mode === 'speed' 
    ? left.avgInterval < right.avgInterval && left.avgInterval > 0
    : left.accuracy > right.accuracy
  const rightIsBetter = mode === 'speed'
    ? right.avgInterval < left.avgInterval && right.avgInterval > 0
    : right.accuracy > left.accuracy
  const isBalanced = leftValue === rightValue
  
  // Calculate bar widths based on relative performance (one decimal, guaranteed sum to 100)
  let leftPercent, rightPercent
  if (mode === 'speed') {
    const total = left.avgInterval + right.avgInterval
    if (total === 0) {
      leftPercent = 50
      rightPercent = 50
    } else {
      // Invert: faster (lower ms) should get MORE bar
      const leftRaw = (right.avgInterval / total) * 100
      leftPercent = Math.round(leftRaw * 10) / 10
      rightPercent = Math.round((100 - leftPercent) * 10) / 10
    }
  } else {
    const total = left.accuracy + right.accuracy
    if (total === 0) {
      leftPercent = 50
      rightPercent = 50
    } else {
      const leftRaw = (left.accuracy / total) * 100
      leftPercent = Math.round(leftRaw * 10) / 10
      rightPercent = Math.round((100 - leftPercent) * 10) / 10
    }
  }
  
  // Get relevant best/worst based on mode
  const best = mode === 'speed' ? bestSpeed : bestAccuracy
  const worst = mode === 'speed' ? worstSpeed : worstAccuracy
  
  // Hand SVG component
  const HandSVG = ({ isLeft, isBetter }) => (
    <svg viewBox="0 0 540.501 640.304" className="hb-hand-svg" style={isLeft ? { transform: 'scaleX(-1)' } : {}}>
      <g transform="matrix(0.8746401,0,0,-0.8518511,33.878521,592.87107)">
        <polyline 
          points={HAND_POLYLINE_1} 
          fill="none" 
          stroke={isBetter ? 'var(--fast)' : '#4a4a4a'} 
          strokeWidth="4" 
          strokeLinecap="square" 
          strokeMiterlimit="10" 
        />
        <polyline 
          points={HAND_POLYLINE_2} 
          fill="none" 
          stroke={isBetter ? 'var(--fast)' : '#4a4a4a'} 
          strokeWidth="4" 
          strokeLinecap="square" 
  ...
  return (
    <div className="hand-balance">
      <div className="hand-balance-header">
        <span className="hb-title">Hand Balance</span>
        <div className="hand-balance-header-right">
          ...
          <Tooltip content={HAND_BALANCE_TIP}>
      ...
      <div className="hand-balance-content">
        {/* Left hand */}
        <div className="hb-hand-section">
          <HandSVG isLeft={true} isBetter={leftIsBetter} />
          <div className="hb-hand-stats">
            <span className="hb-hand-label">left</span>
            <span className={`hb-hand-value ${leftIsBetter ? 'better' : ''}`}>
              {mode === 'speed' ? `${left.avgInterval}ms` : `${formatAccuracy(left.accuracy)}%`}
            </span>
          </div>
        </div>
        
        {/* Balance bar */}
        <div className="hb-bar-section">
          <div className="hb-bar">
            <div 
              className="hb-bar-fill left" 
              style={{ 
                width: `${leftPercent}%`, 
                background: leftIsBetter ? 'var(--fast)' : 'var(--bg-tertiary)' 
              }} 
            />
            <div 
              className="hb-bar-fill right" 
              style={{ 
                width: `${rightPercent}%`, 
                background: rightIsBetter ? 'var(--fast)' : 'var(--bg-tertiary)' 
              }} 
            />
            <div className="hb-bar-center" />
          </div>
          <div className="hb-bar-labels">
        ...
        {/* Right hand */}
        <div className="hb-hand-section">
          <HandSVG isLeft={false} isBetter={rightIsBetter} />
          <div className="hb-hand-stats">
            <span className="hb-hand-label">right</span>
            <span className={`hb-hand-value ${rightIsBetter ? 'better' : ''}`}>
              {mode === 'speed' ? `${right.avgInterval}ms` : `${formatAccuracy(right.accuracy)}%`}
            </span>
          </div>
        </div>
      </div>
      ...
      {!isBalanced && (
        <div className="hb-difference">
          {mode === 'speed' ? (
            <>
              {leftIsBetter ? 'left' : 'right'} hand is{' '}
              <span className="hb-diff-value">
                {Math.abs(left.avgInterval - right.avgInterval)}ms faster
              </span>
            </>
          ) : (
            <>
              {leftIsBetter ? 'left' : 'right'} hand is{' '}
              <span className="hb-diff-value">
                {formatAccuracy(Math.abs(left.accuracy - right.accuracy))}% more accurate
              </span>
            </>
          )}
        </div>
      ...
      {isBalanced && (
        <div className="hb-difference balanced">perfectly balanced</div>
      )}
      
      {/* Best/Worst fingers section */}
      <div className="hb-fingers-section">
        <div className="hb-finger-row">
          <div className="hb-finger-stat">
            <span className="hb-finger-label">
              {mode === 'speed' ? 'fastest left' : 'most accurate left'}
            </span>
            {best.left && (
              <span className="hb-finger-value good">
                {best.left.name} · {mode === 'speed' ? `${best.left.avgInterval}ms` : `${formatAccuracy(best.left.accuracy)}%`}
              </span>
            )}
          </div>
          <div className="hb-finger-stat right">
            <span className="hb-finger-label">
              {mode === 'speed' ? 'fastest right' : 'most accurate right'}
            </span>
            {best.right && (
              <span className="hb-finger-value good">
                {best.right.name} · {mode === 'speed' ? `${best.right.avgInterval}ms` : `${formatAccuracy(best.right.accuracy)}%`}
              </span>
            )}
          </div>
        </div>
        <div className="hb-finger-row">
          <div className="hb-finger-stat">
            <span className="hb-finger-label">
              {mode === 'speed' ? 'slowest left' : 'least accurate left'}
            </span>
            {worst.left && (
              <span className="hb-finger-value bad">
                {worst.left.name} · {mode === 'speed' ? `${worst.left.avgInterval}ms` : `${formatAccuracy(worst.left.accuracy)}%`}
              </span>
            )}
          </div>
          <div className="hb-finger-stat right">
            <span className="hb-finger-label">
              {mode === 'speed' ? 'slowest right' : 'least accurate right'}
            </span>
            {worst.right && (
              <span className="hb-finger-value bad">
                {worst.right.name} · {mode === 'speed' ? `${worst.right.avgInterval}ms` : `${formatAccuracy(worst.right.accuracy)}%`}
              </span>
            )}
          </div>
        </div>
